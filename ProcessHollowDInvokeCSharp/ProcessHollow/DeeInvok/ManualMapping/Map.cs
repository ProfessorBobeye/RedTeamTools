...
namespace DeeInvok.ManualMapping
{
    ...
    public class Map
    {
        ...
        public static SharedData.PE.PE_MANUAL_MAP MapModuleFromDiskToSection(string DLLPath)
        {
            // Check file exists
            ...
            // Open file handle
            SharedData.Native.UNICODE_STRING ObjectName = new SharedData.Native.UNICODE_STRING();
            DynamicInvocation.Native.RtlInitUnicodeString(ref ObjectName, (@"\??\" + DLLPath));
            IntPtr pObjectName = Marshal.AllocHGlobal(Marshal.SizeOf(ObjectName));
            Marshal.StructureToPtr(ObjectName, pObjectName, true);

            SharedData.Native.OBJECT_ATTRIBUTES objectAttributes = new SharedData.Native.OBJECT_ATTRIBUTES();
            ...
            SharedData.Native.IO_STATUS_BLOCK ioStatusBlock = new SharedData.Native.IO_STATUS_BLOCK();

            IntPtr hFile = IntPtr.Zero;
            DynamicInvocation.Native.NtOpenFile(
                ref hFile,
                SharedData.Win32.Kernel32.FileAccessFlags.FILE_READ_DATA |
                SharedData.Win32.Kernel32.FileAccessFlags.FILE_EXECUTE |
                SharedData.Win32.Kernel32.FileAccessFlags.FILE_READ_ATTRIBUTES |
                SharedData.Win32.Kernel32.FileAccessFlags.SYNCHRONIZE,
                ref objectAttributes, ref ioStatusBlock,
                SharedData.Win32.Kernel32.FileShareFlags.FILE_SHARE_READ |
                SharedData.Win32.Kernel32.FileShareFlags.FILE_SHARE_DELETE,
                SharedData.Win32.Kernel32.FileOpenFlags.FILE_SYNCHRONOUS_IO_NONALERT |
                SharedData.Win32.Kernel32.FileOpenFlags.FILE_NON_DIRECTORY_FILE
            );

            // Create section from hFile
            IntPtr hSection = IntPtr.Zero;
            ulong MaxSize = 0;
            SharedData.Native.NTSTATUS ret = DynamicInvocation.Native.NtCreateSection(
                ref hSection,
                (UInt32)SharedData.Win32.WinNT.ACCESS_MASK.SECTION_ALL_ACCESS,
                IntPtr.Zero,
                ref MaxSize,
                SharedData.Win32.WinNT.PAGE_READONLY,
                SharedData.Win32.WinNT.SEC_IMAGE,
                hFile
            );

            // Map view of file
            IntPtr pBaseAddress = IntPtr.Zero;
            DynamicInvocation.Native.NtMapViewOfSection(
                hSection, (IntPtr)(-1), ref pBaseAddress,
                IntPtr.Zero, IntPtr.Zero, IntPtr.Zero,
                ref MaxSize, 0x2, 0x0,
                SharedData.Win32.WinNT.PAGE_READWRITE
            );

            // Prepare return object
            SharedData.PE.PE_MANUAL_MAP SecMapObject = new SharedData.PE.PE_MANUAL_MAP
            {
                PEINFO = DynamicInvocation.Generic.GetPeMetaData(pBaseAddress),
                ModuleBase = pBaseAddress
            };

            DynamicInvocation.Win32.CloseHandle(hFile);
        ...
        public static void RelocateModule(SharedData.PE.PE_META_DATA PEINFO, IntPtr ModuleMemoryBase)
        {
            SharedData.PE.IMAGE_DATA_DIRECTORY idd = PEINFO.Is32Bit ? PEINFO.OptHeader32.BaseRelocationTable : PEINFO.OptHeader64.BaseRelocationTable;
            ...
            {
                SharedData.PE.IMAGE_BASE_RELOCATION ibr = new SharedData.PE.IMAGE_BASE_RELOCATION();
                ibr = (SharedData.PE.IMAGE_BASE_RELOCATION)Marshal.PtrToStructure(pRelocTable, typeof(SharedData.PE.IMAGE_BASE_RELOCATION));
        ...
        /// <param name="ModuleMemoryBase">Base address of the module in memory.</param>
        /// <returns>void</returns>
        public static void RewriteModuleIAT(SharedData.PE.PE_META_DATA PEINFO, IntPtr ModuleMemoryBase)
        {
            SharedData.PE.IMAGE_DATA_DIRECTORY idd = PEINFO.Is32Bit ? PEINFO.OptHeader32.ImportTable : PEINFO.OptHeader64.ImportTable;
            ...
            SharedData.Native.OSVERSIONINFOEX OSVersion = new SharedData.Native.OSVERSIONINFOEX();
            DynamicInvocation.Native.RtlGetVersion(ref OSVersion);
            Dictionary<string, string> ApiSetDict = new Dictionary<string, string>();
            if (OSVersion.MajorVersion >= 10)
            {
                ApiSetDict = DynamicInvocation.Generic.GetApiSetMapping();
            }

            // Loop IID's
            int counter = 0;
            SharedData.Win32.Kernel32.IMAGE_IMPORT_DESCRIPTOR iid = new SharedData.Win32.Kernel32.IMAGE_IMPORT_DESCRIPTOR();
            iid = (SharedData.Win32.Kernel32.IMAGE_IMPORT_DESCRIPTOR)Marshal.PtrToStructure(
                (IntPtr)((UInt64)pImportTable + (uint)(Marshal.SizeOf(iid) * counter)),
                typeof(SharedData.Win32.Kernel32.IMAGE_IMPORT_DESCRIPTOR)
            );
            while (iid.Name != 0)
            {
                // Get DLL
                ...
                else
                {
                    ...
                    IntPtr hModule = DynamicInvocation.Generic.GetLoadedModuleAddress(DllName);
                    if (hModule == IntPtr.Zero)
                    {
                        hModule = DynamicInvocation.Generic.LoadModuleFromDisk(DllName);
                        if (hModule == IntPtr.Zero)
                        {
                    ...
                    if (PEINFO.Is32Bit)
                    {
                        SharedData.PE.IMAGE_THUNK_DATA32 oft_itd = new SharedData.PE.IMAGE_THUNK_DATA32();
                        for (int i = 0; true; i++)
                        {
                            oft_itd = (SharedData.PE.IMAGE_THUNK_DATA32)Marshal.PtrToStructure((IntPtr)((UInt64)ModuleMemoryBase + iid.OriginalFirstThunk + (UInt32)(i * (sizeof(UInt32)))), typeof(SharedData.PE.IMAGE_THUNK_DATA32));
                            ...
                            if (oft_itd.AddressOfData < 0x80000000) // !IMAGE_ORDINAL_FLAG32
                            {
                                ...
                                pFunc = DynamicInvocation.Generic.GetNativeExportAddress(hModule, Marshal.PtrToStringAnsi(pImpByName));
                            ...
                            else
                            {
                                ...
                                IntPtr pFunc = IntPtr.Zero;
                                pFunc = DynamicInvocation.Generic.GetNativeExportAddress(hModule, (short)fOrdinal);
                    ...
                    {
                        SharedData.PE.IMAGE_THUNK_DATA64 oft_itd = new SharedData.PE.IMAGE_THUNK_DATA64();
                        for (int i = 0; true; i++)
                        {
                            oft_itd = (SharedData.PE.IMAGE_THUNK_DATA64)Marshal.PtrToStructure((IntPtr)((UInt64)ModuleMemoryBase + iid.OriginalFirstThunk + (UInt64)(i * (sizeof(UInt64)))), typeof(SharedData.PE.IMAGE_THUNK_DATA64));
        ...
        /// <param name="ModuleMemoryBase">Base address of the module in memory.</param>
        /// <returns>void</returns>
        public static void SetModuleSectionPermissions(SharedData.PE.PE_META_DATA PEINFO, IntPtr ModuleMemoryBase)
        {
            ...
            DynamicInvocation.Native.NtProtectVirtualMemory((IntPtr)(-1), ref ModuleMemoryBase, ref BaseOfCode, SharedData.Win32.WinNT.PAGE_READONLY);

            // Apply section permissions
            foreach (SharedData.PE.IMAGE_SECTION_HEADER ish in PEINFO.Sections)
            {
                bool isRead = (ish.Characteristics & SharedData.PE.DataSectionFlags.MEM_READ) != 0;
                bool isWrite = (ish.Characteristics & SharedData.PE.DataSectionFlags.MEM_WRITE) != 0;
                bool isExecute = (ish.Characteristics & SharedData.PE.DataSectionFlags.MEM_EXECUTE) != 0;
                uint flNewProtect = 0;
                if (isRead & !isWrite & !isExecute)
                {
                    flNewProtect = SharedData.Win32.WinNT.PAGE_READONLY;
                }
                else if (isRead & isWrite & !isExecute)
                {
                    flNewProtect = SharedData.Win32.WinNT.PAGE_READWRITE;
                }
                else if (isRead & isWrite & isExecute)
                {
                    flNewProtect = SharedData.Win32.WinNT.PAGE_EXECUTE_READWRITE;
                }
                else if (isRead & !isWrite & isExecute)
                {
                    flNewProtect = SharedData.Win32.WinNT.PAGE_EXECUTE_READ;
                }
                else if (!isRead & !isWrite & isExecute)
                {
                    flNewProtect = SharedData.Win32.WinNT.PAGE_EXECUTE;
                }
                else
                {
                ...
                // Set protection
                DynamicInvocation.Native.NtProtectVirtualMemory((IntPtr)(-1), ref pVirtualSectionBase, ref ProtectSize, flNewProtect);
            }
        }
        ...
        public static SharedData.PE.PE_MANUAL_MAP MapModuleToMemory(string ModulePath)
        {
        ...
        /// <returns>PE_MANUAL_MAP object</returns>
        public static SharedData.PE.PE_MANUAL_MAP MapModuleToMemory(byte[] Module)
        {
            // Alloc module into memory for parsing
        ...
        /// <returns>PE_MANUAL_MAP object</returns>
        public static SharedData.PE.PE_MANUAL_MAP MapModuleToMemory(byte[] Module, IntPtr pImage)
        {
            // Alloc module into memory for parsing
            IntPtr pModule = AllocateBytesToMemory(Module);
        ...
        /// <returns>PE_MANUAL_MAP object</returns>
        public static SharedData.PE.PE_MANUAL_MAP MapModuleToMemory(IntPtr pModule)
        {
            // Fetch PE meta data
            SharedData.PE.PE_META_DATA PEINFO = DynamicInvocation.Generic.GetPeMetaData(pModule);
            ...
            IntPtr pImage = DynamicInvocation.Native.NtAllocateVirtualMemory(
                (IntPtr)(-1), ref BaseAddress, IntPtr.Zero, ref RegionSize,
                SharedData.Win32.Kernel32.MEM_COMMIT | SharedData.Win32.Kernel32.MEM_RESERVE,
                SharedData.Win32.WinNT.PAGE_READWRITE
            );
        ...
        /// <returns>PE_MANUAL_MAP object</returns>
        public static SharedData.PE.PE_MANUAL_MAP MapModuleToMemory(IntPtr pModule, IntPtr pImage)
        {
            SharedData.PE.PE_META_DATA PEINFO = DynamicInvocation.Generic.GetPeMetaData(pModule);
            return MapModuleToMemory(pModule, pImage, PEINFO);
        }
        ...
        /// <returns>PE_MANUAL_MAP object</returns>
        public static SharedData.PE.PE_MANUAL_MAP MapModuleToMemory(IntPtr pModule, IntPtr pImage, SharedData.PE.PE_META_DATA PEINFO)
        {
            // Check module matches the process architecture
            if ((PEINFO.Is32Bit && IntPtr.Size == 8) || (!PEINFO.Is32Bit && IntPtr.Size == 4))
            {
            ...
            UInt32 BytesWritten = DynamicInvocation.Native.NtWriteVirtualMemory((IntPtr)(-1), pImage, pModule, SizeOfHeaders);
            ...
            foreach (SharedData.PE.IMAGE_SECTION_HEADER ish in PEINFO.Sections)
            {
                ...
                // Write data
                BytesWritten = DynamicInvocation.Native.NtWriteVirtualMemory((IntPtr)(-1), pVirtualSectionBase, pRawSectionBase, ish.SizeOfRawData);
            ...
            // Prepare return object
            SharedData.PE.PE_MANUAL_MAP ManMapObject = new SharedData.PE.PE_MANUAL_MAP
            {
                ModuleBase = pImage,
                PEINFO = PEINFO
            };
        ...
        public static void FreeModule(SharedData.PE.PE_MANUAL_MAP PEMapped)
        {
            // Check if PE was mapped via module overloading
            if (!string.IsNullOrEmpty(PEMapped.DecoyModule))
            {
                DynamicInvocation.Native.NtUnmapViewOfSection((IntPtr)(-1), PEMapped.ModuleBase);
            }
            // If PE not mapped via module overloading, free the memory.
            else
            {
                SharedData.PE.PE_META_DATA PEINFO = PEMapped.PEINFO;
                ...
                DynamicInvocation.Native.NtFreeVirtualMemory((IntPtr)(-1), ref pModule, ref size, SharedData.Win32.Kernel32.MEM_RELEASE);
            }
        }
    }
}
