...
using ManualMap = DeeInvok.ManualMapping;
...
namespace DeeInvok.DynamicInvocation
{
    ...
    public class Generic
    {
        ...
        {
            SharedData.Native.UNICODE_STRING uModuleName = new SharedData.Native.UNICODE_STRING();
            Native.RtlInitUnicodeString(ref uModuleName, DLLPath);

            IntPtr hModule = IntPtr.Zero;
            SharedData.Native.NTSTATUS CallResult = Native.LdrLoadDll(IntPtr.Zero, 0, ref uModuleName, ref hModule);
            if (CallResult != SharedData.Native.NTSTATUS.Success || hModule == IntPtr.Zero)
            {
                return IntPtr.Zero;
            }
        ...
        {
            // Get _PEB pointer
            SharedData.Native.PROCESS_BASIC_INFORMATION pbi = Native.NtQueryInformationProcessBasicInformation((IntPtr)(-1));
            ...
            SharedData.Native.LIST_ENTRY le = (SharedData.Native.LIST_ENTRY)Marshal.PtrToStructure(pInLoadOrderModuleList, typeof(SharedData.Native.LIST_ENTRY));

            // Loop entries
            IntPtr flink = le.Flink;
            IntPtr hModule = IntPtr.Zero;
            SharedData.PE.LDR_DATA_TABLE_ENTRY dte = (SharedData.PE.LDR_DATA_TABLE_ENTRY)Marshal.PtrToStructure(flink, typeof(SharedData.PE.LDR_DATA_TABLE_ENTRY));
            while (dte.InLoadOrderLinks.Flink != le.Blink)
            {
                ...
                dte = (SharedData.PE.LDR_DATA_TABLE_ENTRY)Marshal.PtrToStructure(flink, typeof(SharedData.PE.LDR_DATA_TABLE_ENTRY));
            }
        ...
        {
            SharedData.Native.ANSI_STRING aFunc = new SharedData.Native.ANSI_STRING
            {
        ...
        public static SharedData.PE.PE_META_DATA GetPeMetaData(IntPtr pModule)
        {
            SharedData.PE.PE_META_DATA PeMetaData = new SharedData.PE.PE_META_DATA();
            try
            {
                ...
                PeMetaData.ImageFileHeader = (SharedData.PE.IMAGE_FILE_HEADER)Marshal.PtrToStructure((IntPtr)((UInt64)pModule + e_lfanew + 0x4), typeof(SharedData.PE.IMAGE_FILE_HEADER));
                ...
                if (PEArch == 0x010b) // Image is x32
                {
                    ...
                    PeMetaData.OptHeader32 = (SharedData.PE.IMAGE_OPTIONAL_HEADER32)Marshal.PtrToStructure(OptHeader, typeof(SharedData.PE.IMAGE_OPTIONAL_HEADER32));
                }
                else if (PEArch == 0x020b) // Image is x64
                {
                    PeMetaData.Is32Bit = false;
                    PeMetaData.OptHeader64 = (SharedData.PE.IMAGE_OPTIONAL_HEADER64)Marshal.PtrToStructure(OptHeader, typeof(SharedData.PE.IMAGE_OPTIONAL_HEADER64));
                } else
                {
                    throw new InvalidOperationException("Invalid magic value (PE32/PE32+).");
                }
                // Read sections
                SharedData.PE.IMAGE_SECTION_HEADER[] SectionArray = new SharedData.PE.IMAGE_SECTION_HEADER[PeMetaData.ImageFileHeader.NumberOfSections];
                for (int i = 0; i < PeMetaData.ImageFileHeader.NumberOfSections; i++)
                {
                    ...
                    SectionArray[i] = (SharedData.PE.IMAGE_SECTION_HEADER)Marshal.PtrToStructure(SectionPtr, typeof(SharedData.PE.IMAGE_SECTION_HEADER));
                }
        ...
        public static Dictionary<string, string> GetApiSetMapping()
        {
            ...
            SharedData.PE.ApiSetNamespace Namespace = (SharedData.PE.ApiSetNamespace)Marshal.PtrToStructure(pApiSetNamespace, typeof(SharedData.PE.ApiSetNamespace));
            for (var i = 0; i < Namespace.Count; i++)
            {
                SharedData.PE.ApiSetNamespaceEntry SetEntry = new SharedData.PE.ApiSetNamespaceEntry();
                IntPtr pSetEntry = (IntPtr)((UInt64)pApiSetNamespace + (UInt64)Namespace.EntryOffset + (UInt64)(i * Marshal.SizeOf(SetEntry)));
                SetEntry = (SharedData.PE.ApiSetNamespaceEntry)Marshal.PtrToStructure(pSetEntry, typeof(SharedData.PE.ApiSetNamespaceEntry));
                ...
                SharedData.PE.ApiSetValueEntry SetValue = new SharedData.PE.ApiSetValueEntry();
                ...
                SetValue = (SharedData.PE.ApiSetValueEntry)Marshal.PtrToStructure(pSetValue, typeof(SharedData.PE.ApiSetValueEntry));
        ...
        public static void CallMappedPEModule(SharedData.PE.PE_META_DATA PEINFO, IntPtr ModuleMemoryBase)
        {
            ...
            Native.NtCreateThreadEx(
                ref hRemoteThread,
                SharedData.Win32.WinNT.ACCESS_MASK.STANDARD_RIGHTS_ALL,
        ...
        /// <param name="ModuleMemoryBase">Base address of the module in memory.</param>
        /// <returns>void</returns>
        public static void CallMappedDLLModule(SharedData.PE.PE_META_DATA PEINFO, IntPtr ModuleMemoryBase)
        {
            ...
            {
                SharedData.PE.DllMain fDllMain = (SharedData.PE.DllMain)Marshal.GetDelegateForFunctionPointer(lpEntryPoint, typeof(SharedData.PE.DllMain));
                try
                {
                    bool CallRes = fDllMain(ModuleMemoryBase, SharedData.PE.DLL_PROCESS_ATTACH, IntPtr.Zero);
                    if (!CallRes)
                    {
        ...
        /// <returns>void</returns>
        public static object CallMappedDLLModuleExport(SharedData.PE.PE_META_DATA PEINFO, IntPtr ModuleMemoryBase, string ExportName, Type FunctionDelegateType, object[] Parameters, bool CallEntry = true)
        {
        ...
        /// <param name="CallEntry">Specify whether to invoke the module's entry point.</param>
        /// <returns>void</returns>
        public static object CallMappedDLLModuleExport(SharedData.PE.PE_META_DATA PEINFO, IntPtr ModuleMemoryBase, short Ordinal, Type FunctionDelegateType, object[] Parameters, bool CallEntry = true)
        {
            // Call entry point if user has specified
            if (CallEntry)
            {
        ...
        /// <param name="CallEntry">Specify whether to invoke the module's entry point.</param>
        /// <returns>void</returns>
        public static object CallMappedDLLModuleExport(SharedData.PE.PE_META_DATA PEINFO, IntPtr ModuleMemoryBase, string FunctionHash, long Key, Type FunctionDelegateType, object[] Parameters, bool CallEntry = true)
        {
            // Call entry point if user has specified
            if (CallEntry)
            {
        ...
        public static IntPtr GetSyscallStub(string FunctionName)
        {
            ...
            IntPtr pModule = ManualMapping.Map.AllocateFileToMemory(NtdllPath);

            // Fetch PE meta data
            SharedData.PE.PE_META_DATA PEINFO = GetPeMetaData(pModule);
            ...
            Native.NtProtectVirtualMemory((IntPtr)(-1), ref pCallStub, ref RegionSize, SharedData.Win32.WinNT.PAGE_EXECUTE_READ);

            // Free temporary allocations
            Marshal.FreeHGlobal(pModule);
            RegionSize = PEINFO.Is32Bit ? (IntPtr)PEINFO.OptHeader32.SizeOfImage : (IntPtr)PEINFO.OptHeader64.SizeOfImage;

            Native.NtFreeVirtualMemory((IntPtr)(-1), ref pImage, ref RegionSize, SharedData.Win32.Kernel32.MEM_RELEASE);
