...
namespace PaupDynamo.ByHandMap {
    ...
    public class Map {
        ...
        public static void RewriteModuleIAT(Data.PE.PE_META_DATA PEINFO, IntPtr ModuleMemoryBase) {
            ...
            Dynamo.Native.RtlGetVersion(ref OSVersion);
            Dictionary<string, string> ApiSetDict = new Dictionary<string, string>();
            if (OSVersion.MajorVersion >= 10) {
                ApiSetDict = Dynamo.Generic.GetApiSetMapping();
            }
            ...
            while (iid.Name != 0) {
                ...
                if (DllName == string.Empty) {
                    ...
                    IntPtr hModule = Dynamo.Generic.GetLoadedModuleAddress(DllName);
                    if (hModule == IntPtr.Zero) {
                        hModule = Dynamo.Generic.LoadModuleFromDisk(DllName);
                        if (hModule == IntPtr.Zero) {
                    ...
                    if (PEINFO.Is32Bit) {
                        ...
                        for (int i = 0; true; i++) {
                            ...
                            if (oft_itd.AddressOfData < 0x80000000) // !IMAGE_ORDINAL_FLAG32
                            {
                                ...
                                pFunc = Dynamo.Generic.GetNativeExportAddress(hModule, Marshal.PtrToStringAnsi(pImpByName));
                                ...
                                IntPtr pFunc = IntPtr.Zero;
                                pFunc = Dynamo.Generic.GetNativeExportAddress(hModule, (short)fOrdinal);
        ...
        public static void SetModuleSectionPermissions(Data.PE.PE_META_DATA PEINFO, IntPtr ModuleMemoryBase) {
            ...
            Dynamo.Native.NtProtectVirtualMemory((IntPtr)(-1), ref ModuleMemoryBase, ref BaseOfCode, Data.Win32.WinNT.PAGE_READONLY);
            ...
            foreach (Data.PE.IMAGE_SECTION_HEADER ish in PEINFO.Sections) {
                ...
                // Set protection
                Dynamo.Native.NtProtectVirtualMemory((IntPtr)(-1), ref pVirtualSectionBase, ref ProtectSize, flNewProtect);
            }
        }
        ...
        public static Data.PE.PE_MANUAL_MAP MapModuleToMemory(IntPtr pModule) {
            // Fetch PE meta data
            Data.PE.PE_META_DATA PEINFO = Dynamo.Generic.GetPeMetaData(pModule);
            ...
            IntPtr pImage = Dynamo.Native.NtAllocateVirtualMemory(
        ...
        public static Data.PE.PE_MANUAL_MAP MapModuleToMemory(IntPtr pModule, IntPtr pImage, Data.PE.PE_META_DATA PEINFO) {
            ...
            UInt32 BytesWritten = Dynamo.Native.NtWriteVirtualMemory((IntPtr)(-1), pImage, pModule, SizeOfHeaders);
            ...
            foreach (Data.PE.IMAGE_SECTION_HEADER ish in PEINFO.Sections) {
                ...
                // Write data
                BytesWritten = Dynamo.Native.NtWriteVirtualMemory((IntPtr)(-1), pVirtualSectionBase, pRawSectionBase, ish.SizeOfRawData);
        ...
        public static void FreeModule(Data.PE.PE_MANUAL_MAP PEMapped) {
            ...
            if (!string.IsNullOrEmpty(PEMapped.DecoyModule)) {
                Dynamo.Native.NtUnmapViewOfSection((IntPtr)(-1), PEMapped.ModuleBase);
            }
            // If PE not mapped via module overloading, free the memory.
                ...
                Dynamo.Native.NtFreeVirtualMemory((IntPtr)(-1), ref pModule, ref size, Data.Win32.Kernel32.MEM_RELEASE);
            }
        }
    }
}
